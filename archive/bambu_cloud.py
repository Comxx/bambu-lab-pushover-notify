...
class BambuCloud:
    ...
    def _get_authentication_token(self) -> dict:
        ...
        response = requests.post(url, json=data, timeout=10, verify=certifi.where())
        if not response.ok:
            logging.debug(f"Received error: {response.status_code}")
    ...
    def get_latest_task_for_printer(self, deviceId: str) -> dict:
        logging.debug(f"Getting latest task from Bambu Cloud for Printer: {deviceId}")
        data = self.get_tasklist_for_printer(deviceId)
        if len(data) != 0:
            return data[0]
        logging.debug("No tasks found for printer")
        return None

    def get_tasklist_for_printer(self, deviceId: str) -> dict:
        logging.debug(f"Getting task list from Bambu Cloud for Printer: {deviceId}")
        tasks = []
