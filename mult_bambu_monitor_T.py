...
# Dictionary to store BambuCloud authentication details for each printer
auth_details = {}
...
def search_error(error_code, english_errors):
    try:
            ...
            for thread in threads:
                if not thread.is_alive():
                    ...
                    if thread.name == flask_thread.name:
                        ...
                        broker = next(b for b in brokers if b["device_id"] in thread.name)
