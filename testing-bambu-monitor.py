...
def on_message(client, userdata, msg, attr, code):
    ...
    if response.status_code == 200:
        ...
        # Extracting English error codes and descriptions from site
        ...
        def search_error(error_code, error_list):
            for error in error_list:
                if error["ecode"] == error_code:
                    return error
            return None
        hms_data = dataDict['print']['hms']
        # Example: Searching for device_hms error code
        device_error_code_to_search = [hms_code(entry['attr'], entry['code']) for entry in hms_data]
       # error_code_to_hms_cleaned = device_error_code_to_search.replace("hms_", "")
        ...
        found_device_error = search_error(device_error_code_to_search, english_errors)

        if found_device_error:
            logging.info("Device Error Code:", device_error_code_to_search)
            logging.info("Description:", found_device_error["intro"])
            logging.info("URL:", f"https://wiki.bambulab.com/en/x1/troubleshooting/hmscode/{error_code_to_hms_cleaned}")
        else:
            logging.info("Device error code", device_error_code_to_search, "not found.")
    else:
        logging.info("Failed to fetch data:", response.status_code)
        ...
        if 'gcode_state' in dataDict['print']:
            ...
            if gcode_state_prev != gcode_state or (gcode_state_prev != gcode_state and not percent_notify and percent_done >= notify_at_percent):
                ...
                if ('fail_reason' in dataDict['print'] and len(dataDict['print']['fail_reason']) > 1) or ('print_error' in dataDict['print'] and dataDict['print']['print_error'] != 0) or gcode_state == "FAILED":
                    ...
                    msg_text = msg_text + f"<li>HMS code: {device_error_code_to_search}</li>"
                    msg_text = msg_text + f"<li>Description: {found_device_error['intro']}</li>"
                    msg_text = msg_text + f"<li>URL: https://wiki.bambulab.com/en/x1/troubleshooting/hmscode/{error_code_to_hms_cleaned}</li>"
